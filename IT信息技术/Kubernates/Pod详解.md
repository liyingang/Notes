...
-   Pause容器，这是每个Pod都会有的一个根容器，它的作用有两个：
    ...
    +   可以以它为依据，评估整个Pod的健康状态，主要负责僵尸进程的回收管理
...
metadata:       　 #必选，元数据
  name: string     #必选，符合RFC 1035规范的 Pod名称
  namespace: string  #可选，Pod所属的命名空间，不指定默认为default，可以使用-n 指定namespace
...
spec:  #必选，Pod中容器的详细定义
  ...
  - name: string   #必选，容器名称
    image: string  #必选，容器的镜像名称，即镜像的地址
...
![在这里插入图片描述](Kubernetes使用/7be963e27cbc4d138e1b9734c58bcad3.png)
...
![在这里插入图片描述](Kubernetes使用/b37a2a1680454c318bc9ef719751c861.png)
...
![在这里插入图片描述](Kubernetes使用/dde43d6f6c4a4f04a7310ac3eea0aa38.png)
...
-   StartupProbe:1.16版本后添加的探测方式，判断容器内程序是否已经启动。如果配置了StartupProbe，就会先禁止其他探测，直到成功为止。
...
+   HTTPGet：调用容器内Web应用的URL，如果返回的状态码在200和399之间，则认为程序正常，否则不正常（程序暴露API地址）
...
![在这里插入图片描述](Kubernetes使用/0cfd7e7198f64af68aca1f2894d3a4ce.png)
...
![在这里插入图片描述](Kubernetes使用/bec001768cfb47ef9d1c05ffa4b75d8e.png)
